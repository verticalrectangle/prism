                                  ...
                                  const RvcConfig& cfg)
{
    ...
    auto phase_cs = g.emit("CumSum", {freq, cs_ax}, {G::attr_int("exclusive", 0)}, "sg_phase");
    // Streaming: offset by the caller-tracked oscillator phase ("phase" graph
    // input, [1]) so consecutive chunks continue the sine instead of
    // restarting at 0 — a 0-phase restart every hop buzzes at the hop rate.
    auto phase   = op_add(g, phase_cs, "phase", "sg_ph0");
...
std::string pth_to_onnx(const PthModel& m, const std::string& out_path)
{
    ...
    try {
        ...
        // (~/.cache/prism/models/rvc_pitch_bases.bin) reveals exactly which
        // pitch bins the voice was trained on. The mask drives auto-octave in
        // the ML thread at convert time.
        std::string mask_hex;
        if (cfg.phone_dim == 768 &&
            m.tensors.count("enc_p.emb_pitch.weight")) {
            auto ft = tl.load("enc_p.emb_pitch.weight");      // [256, hid]
            auto sh = tl.shape("enc_p.emb_pitch.weight");
            const char* home = getenv("HOME");
            std::string bp = std::string(home ? home : "") +
                             "/.cache/prism/models/rvc_pitch_bases.bin";
            ...
            if (bf && bf.read(magic, 7) && std::memcmp(magic, "PMSPB1\n", 7) == 0) {
                while (bf) {
                    ...
                    float peak = 0.f;
                    for (uint32_t b = 0; b < rows; b++) {
                        int lo = (int)b - 4, hi = (int)b + 4, cnt = 0;
                        float acc = 0.f;
                        for (int k = lo; k <= hi; k++)
                            if (k >= 0 && k < (int)rows) { acc += d[(size_t)k]; cnt++; }
                        sm[b] = acc / (float)cnt;
                        peak  = std::fmax(peak, sm[b]);
                    }
                    // 64 hex chars, bin 0 = LSB of first nibble group
                    for (uint32_t b = 0; b + 3 < rows; b += 4) {
                        int nib = 0;
                        for (int k = 0; k < 4; k++)
